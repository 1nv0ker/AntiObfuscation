...
function getMatomoParams(domain, uuid, type=1) {
    const time = new Date()
    let e_c = ''
    let e_a = ''
    let e_n = ''
    if (type===1) {
        e_c = 'screen' //extension"screen", "view", "opt in
        e_a = 'view'//scroll
        e_n = 'data/iploong.com/non-registered'//100% //data/bitip.com/non-registered
    }
    if (type===2) {
        e_c = 'extension' 
        e_a = 'scroll'
    ...
    let start = `e_c=${e_c}&e_a=${e_a}&e_n=${e_n}&ca=${ca}&idsite=${idsite}&rec=${rec}&r=${r}&h=${h}&m=${m}&s=${s}&url=${url}&_id=${_id}&_idn=${_idn}&_refts=${_refts}&
    send_image=${send_image}&pdf=1&qt=0&realp=0&wma=0&fla=0&java=0&ag=0&cookie=${cookie}&res=${res}
    &dimension11=${uuid}`
    let end = `&pf_net=${pf_net}&pf_srv=${pf_srv}&pf_tfr=${pf_tfr}&pf_dm1=${pf_dm1}&pf_dm2=${pf_dm2}&pf_onl=${pf_onl}`
    if (type === 1) {
        return start + end
    }
    if (type === 2) {
